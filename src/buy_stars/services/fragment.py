    ...
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            response = await client.post(
                f"{FRAGMENT_API_BASE_URL}/order/stars/", headers=HEADERS, json=payload
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Fragment API error: {response.status_code} — {response.text}"
                )
                return False
...
async def buy_premium(username: str, months: int, show_sender: bool = False) -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            response = await client.post(
                f"{FRAGMENT_API_BASE_URL}/order/premium/", headers=HEADERS, json=payload
            )
        ...
        print(
            f"❗ Exception occurred while purchasing premium: {type(e).__name__} — {e}"
        )
        return False

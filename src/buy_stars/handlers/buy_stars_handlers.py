...
async def start_buy(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        t(lang, "buy_stars.username"),
        reply_markup=stars_username_keyboard(lang, username),
    )


@router.callback_query(
    BuyStarsState.waiting_for_username_choice, F.data == "stars_for_self"
)
async def handle_self_username(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        t(lang, "buy_stars.stars_amount"), reply_markup=back_to_menu_keyboard(lang)
    )


@router.callback_query(
    BuyStarsState.waiting_for_username_choice, F.data == "stars_for_other"
)
async def ask_other_username(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        t(lang, "buy_stars.enter_username"), reply_markup=back_to_menu_keyboard(lang)
    )
...
async def save_friend_username(message: Message, state: FSMContext) -> None:
    ...
    if not username:
        await message.answer(
            t(lang, "buy_stars.invalid_username"),
...
async def ask_payment_method(message: Message, state: FSMContext) -> None:
    ...
    if not message.text.isdigit() or int(message.text) < 50:
        await message.answer(
            t(lang, "buy_stars.invalid_amount"),
            reply_markup=back_to_menu_keyboard(lang),
        )
        return
    ...
    await message.answer(
        t(lang, "buy_stars.payment_method"), reply_markup=payment_method_keyboard(lang)
    )


@router.callback_query(
    BuyStarsState.waiting_for_payment_method,
    ...
    if payment_type == PaymentType.TON:
        ...
        if invoice_url:
            ...
            await callback.message.answer(
                t(lang, "buy_stars.error_3"), reply_markup=back_to_menu_keyboard(lang)
            )


@router.callback_query(
    BuyStarsState.waiting_for_confirmation, F.data == "confirm_payment"
)
async def handle_confirm_payment(callback: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    lang = await get_lang(callback.from_user.id)
    await callback.message.answer(
        t(lang, "buy_stars.confirm_payment"), reply_markup=back_to_menu_keyboard(lang)
    )

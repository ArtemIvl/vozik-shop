...
import { cancelMiniAppOrder, createMiniAppPremiumOrder, getMiniAppOrderPaymentLink, getMiniAppPendingOrders } from "../services/api";
...
export default function BuyPremiumPage({ initData, tgUser, sendData, onOrdersUpdated, t }) {
  ...
  const canSubmit = !!targetUsername && !loading;
  ...
  const loadPendingOrders = async () => {
    ...
    try {
      ...
      setWatchedOrderIds((current) => {
        ...
        if (completed) {
          setOverlayMessage(t.premiumOrderSuccess);
        }
  ...
  const handleCreateOrder = async (event) => {
    ...
    try {
      setLoading(true);
      const result = await createMiniAppPremiumOrder({
        init_data: initData,
      ...
      sendData({ event: "miniapp_order_created", orderId: result.orderId, type: "premium" });
      await loadPendingOrders();
    } catch (requestError) {
      setError(requestError.message || t.createPremiumOrder);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="space-y-4">
      ...
      />
      <p className="text-xs uppercase tracking-[0.14em] text-tg-muted">{t.buyPremium}</p>

      <div className="rounded-3xl border border-white/10 bg-tg-surface p-4">
        <h2 className="text-lg font-semibold text-tg-text">{t.createPremiumOrderTitle}</h2>
        <p className="mt-1 text-sm text-tg-muted">{t.premiumFlowHint}</p>

        <form className="mt-4 space-y-4" onSubmit={handleCreateOrder}>
          <div>
            <label className="mb-2 block text-sm text-tg-muted">{t.whoPremiumFor}</label>
            <SegmentedSwitch
              value={recipientMode}
          ...
          <div>
            <label className="mb-2 block text-sm text-tg-muted">{t.duration}</label>
            <div className="grid grid-cols-3 gap-2">
              {PREMIUM_PLANS.map((planMonths) => {
                const active = months === planMonths;
                return (
                  <button
                    key={planMonths}
                    type="button"
                    ...
                    className={`rounded-xl border px-3 py-2 text-sm transition ${
                      active
                        ...
                        : "border-white/15 bg-tg-surface-soft text-tg-muted"
                    }`}
                  >
                    {planMonths} {t.months}
                  </button>
          ...
          >
            {loading ? t.creatingOrder : t.createPremiumOrder}
          </button>
        </form>
      </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-tg-surface p-4">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-base font-semibold text-tg-text">{t.pendingPremium}</h3>
          <button
            type="button"
        ...
        {!pendingLoading && pendingOrders.length === 0 ? (
          <p className="text-sm text-tg-muted">{t.noPendingPremium}</p>
        ) : null}

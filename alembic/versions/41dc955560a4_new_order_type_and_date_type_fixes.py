...
def upgrade() -> None:
    ...
    op.alter_column("orders", "memo", existing_type=sa.VARCHAR(), nullable=False)
    op.alter_column(
        "orders",
        "created_at",
...
def downgrade() -> None:
    op.add_column(
        "withdrawals",
        sa.Column(
            "reg_date", postgresql.TIMESTAMP(), autoincrement=False, nullable=True
        ),
    )
    ...
    op.alter_column("orders", "memo", existing_type=sa.VARCHAR(), nullable=True)
    op.alter_column(
        "orders",
        "payment_type",
        existing_type=postgresql.ENUM("TON", "USDT", name="paymenttype"),
